...
export function LocaleProvider({ initialLocale, children }: LocaleProviderProps): React.JSX.Element {
  ...
  /**
   * Sync locale when the initialLocale prop changes.
   * Uses functional setState to avoid reading the current 'locale' inside
   * the effect. This keeps the dependency array to [initialLocale] and
   * prevents unintended overrides when 'locale' changes elsewhere.
   */
  useEffect(() => {
    const normalized = getBestLocale(initialLocale);
    setLocaleState(prev => (prev !== normalized ? normalized : prev));
