...
**Note:** `.env.example` and `SETUP.portfolio-generated.md` files are automatically generated by the integration script (`./scripts/integrate-clients.sh`) with correct ports from `client.config.json`. You don't need to create these manually.
...
**Note:** The integration script automatically generates `.env.example` and `SETUP.portfolio-generated.md` files for each client. These files use the ports and configuration from `client.config.json`, ensuring consistency across your client setup.
...
- [ ] Environment files (`.env.example` and `SETUP.portfolio-generated.md`) are generated (via `./scripts/integrate-clients.sh`)
