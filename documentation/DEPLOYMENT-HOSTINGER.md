...
Production deploys **only the Next.js landing** (`focus-on-pixel.com`). The VPS pulls a pre-built image from **GHCR** — no Docker build on the host.
...
2. **Deploy is manual** → **Actions → Deploy landing to Hostinger → Run workflow** (after CI is green).
3. Workflow builds the frontend image on GitHub Actions, pushes to GHCR, Hostinger VPS **pulls** and restarts.
4. One-shot **`post-deploy-prune`** removes old `portfolio-frontend` images on the VPS (project-scoped).
...
`GITHUB_TOKEN` is used automatically for GHCR push and registry prune — no extra PAT in GitHub Secrets.

### Private repo

Add a [Hostinger deploy SSH key](https://www.hostinger.com/support/how-to-deploy-from-private-github-repository-on-hostinger-docker-manager/) if Hostinger must clone the repo on the VPS. Runtime images still come from GHCR.
...
Serializes layer downloads during pulls. **Do not re-run** unless you know you need to change `/etc/docker/daemon.json`.
...
- **Path filter**: only rebuilds when `apps/frontend`, shared packages, lockfile, or hostinger compose/workflow changed.
...
- Prune is **scoped** to `ghcr.io/*/portfolio-frontend` and containers named `portfolio` — it does not remove other stacks.
