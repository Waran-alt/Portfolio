...
Quick setup guide for VatMan application.

## Configuration

- **Client ID**: `vat-man`
- **Subdomain**: `vat-man`
- **Full URL**: `https://vat-man.yourdomain.com`
- **Frontend Port**: `3011`
- **Backend Port**: `4011`
- **Database**: `vat_man_db`
...
# =============================================================================
# DATABASE_URL=jdbc:postgresql://localhost:5432/vat_man_db
```
...
- Ports must match the values in `client.config.json` (3011/4011)
...
When you work inside this client directory (e.g. `clients/vat-man/`) but the stack runs from the Portfolio root:

| Task                                        | From vat-man folder                                                   | From Portfolio root                                                       |
|---------------------------------------------|---------------------------------------------------------------------------|---------------------------------------------------------------------------|
| **Rebuild** (after Dockerfile/deps changes) | `../../scripts/rebuild-client.sh` (auto-detects client)                   | `yarn clients:rebuild vat-man`                                            |
| **Rebuild + restart**                       | `../../scripts/rebuild-client.sh --restart`                               | `yarn clients:rebuild vat-man -- --restart`                               |
| **Restart** (no rebuild)                    | `../../scripts/docker-stack.sh restart vat-man-backend vat-man-frontend`  | `./scripts/docker-stack.sh restart vat-man-backend vat-man-frontend`  |
| **Logs**                                    | `../../scripts/docker-stack.sh logs -f vat-man-frontend`                  | `./scripts/docker-stack.sh logs -f vat-man-frontend`                      |
...
- **Production URL**: https://vat-man.yourdomain.com
...
4. **Find database**: Expand **Servers** → your server → **Databases** → `vat_man_db` → **Schemas** → **public** → **Tables**


## Notes

- Ports are configured in `client.config.json`
- Database name is `vat_man_db`
